...
| Site-only propagation failures, share of sampled, weekly max | ≤ 0.5% |
| Snapshot age at capture (now − LEO snapshot `generated_at`), weekly max | ≤ 12 h |
| Daily captures present per week | ≥ 4 of 7 (in the first week, every day since the first capture, up to 4) |

A week in which days are present but no object was measured fails the math check. Element age
(p50, p95, share older than 3 days) is reported for information only: it reflects Space-Track's
tracking cadence, not the site's freshness.

T5's expected vectors come from the Python `sgp4` package running Vallado's SGP4-VER test cases
(generated by `tools/accuracy/make_sgp4_vectors.py`), not from the `tcppver.out` file; T3 compares
the site's own `json2satrec` path (`recordToSatrec`) against skyfield.
...
`AUDIT.md` and updates `history.json`, and
...
| Section | Contents |
|---|---|
| Week table | each check (math error, site-only failures, ISS ground and altitude, snapshot age, days present) with value, limit and PASS/FAIL |
| Missing days | days without a valid capture since the first capture |
| Daily detail (information) | per day: snapshot age (h), sampled, `failures` site / reference / both, element age p50 / p95 and share older than 3 days |
| Worst 10 | the largest ground errors of the week, one row per object |
| 12-week trend | pass/fail, math max and ISS max per week |
