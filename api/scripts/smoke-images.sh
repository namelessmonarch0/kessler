...
cid=$(docker run -d --rm --network host -v "$RIE_DIR:/rie:ro" -e DATABASE_URL="$DB_URL" \
  --entrypoint /rie/aws-lambda-rie kessler-local:jobs python -m awslambdaric app.lambda_jobs.handler)
...
docker run --rm --entrypoint sh kessler-local:api -c 'test -x /opt/extensions/lambda-adapter' \
